![Spin Ribo-CORD banner](spin-ribo-cord-banner_1.png)
...
The pipeline aligns paired-end sequencing reads to a pooled UTR+CDS reference, counts cytosine-to-uracil (C→T) mismatches introduced by Ribo-CORD editing, and computes an Edits Per Read (EPR) metric. It then performs a Welch's t-test between Dox (editor-on) and NoDox (editor-off) replicates to identify UTRs with significantly elevated translational activity.
